...
def process_audio_file(audio_path, text, control_path, polyphone):
    ...
    try:
        ...
        return (audio_path, text, audio_duration, control_path)
...
def prepare_csv_wavs_dir(input_dir, num_workers=None):
    ...
    for (audio_path, _, duration, control_path), conv_text in zip(processed, converted_texts):
        sub_result.append({"audio_path": audio_path, "text": conv_text, "duration": duration, "cond_audio_path": control_path})
    ...
    with open(csv_file_path, mode="r", newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            if len(row) >= 3:
                ...
                audio_text_pairs.append((audio_file_path.as_posix(), text, control_file_path.as_posix()))
