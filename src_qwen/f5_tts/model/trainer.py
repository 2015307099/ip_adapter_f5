        ...
        for epoch in range(skipped_epoch, self.epochs):
            ...
            for batch in current_dataloader:
                with self.accelerator.accumulate(self.model):
                    ...
                    print(f"TRAIN mel_spec shape: {mel_spec.shape}" )
                    mel_lengths = batch["mel_lengths"]
                    

                    # control_cond = batch["cond_mel"].permute(0, 2, 1)
                    control_cond = batch["qwen_feat"]
                    print("TRAIN control_cond shape:", control_cond.shape)
                    ...
                    loss, cond, pred = self.model(
                        mel_spec, text=text_inputs, lens=mel_lengths, noise_scheduler=self.noise_scheduler, control_cond=control_cond
                    )
                    self.accelerator.backward(loss)
                    
                    # debug
                    # if global_update % 5 == 0:  # 没必要每步都打，每 100 步看一眼
                    #     if self.accelerator.is_local_main_process:
                    #         # 获取解包后的原始模型
                    #         unwrapped_model = self.accelerator.unwrap_model(self.model)
                            
                    #         # 1. 检查 ControlNet 层的梯度 (应该有值)
                    #         ctrl_grad = unwrapped_model.transformer.controlnet[0].before_proj.weight.grad
                    #         ctrl_grad_norm = ctrl_grad.norm().item() if ctrl_grad is not None else "None"
                            
                    #         # 2. 检查冻结的 Base Model 层梯度 (应该为 None 或非常接近 0)
                    #         # 注意：如果 F5-TTS 开启了 requires_grad = False，这里通常是 None
                    #         base_grad = unwrapped_model.transformer.base_model.transformer_blocks[0].attn.to_q.weight.grad
                    #         base_grad_norm = base_grad.norm().item() if base_grad is not None else "None"
                            
                    #         print(f"[Gradient Check] Update {global_update}:")
                    #         print(f"  - ControlNet Block 0 Proj Grad Norm: {ctrl_grad_norm}")
                    #         print(f"  - Base Model Block 0 Attn Grad Norm: {base_grad_norm}")
                ...
                if global_update % self.save_per_updates == 0 and self.accelerator.sync_gradients:
                    ...
                    #     print(f"Logging sample at update {global_update}, text: {infer_text}, mel_spec shape: {mel_spec.shape}, control_cond shape: {control_cond.shape}")
                    ...
                    #     with open(f"{log_samples_path}/update_{global_update}_text.txt", "w", encoding="utf-8") as f:
                    #         f.write(infer_text[0] if isinstance(infer_text[0], str) else "".join(infer_text[0]))
                    ...
                    if self.log_samples and self.accelerator.is_local_main_process:
                        ...
                        # ✅ 在这里添加多条待增强音频路径
                        NOISY_AUDIO_LIST = [
                            "/yangliusha03/panyuanhao/136.wav",
                            "/yangliusha03/panyuanhao/139.wav",
                        ]
                        GEN_TEXT = "empty"  # 固定文本统一为empty
                        target_sr = 24000
                        hop_length = 256
                        ...
                        # ===================== 循环生成所有音频 =====================
                        for idx, NOISY_AUDIO in enumerate(NOISY_AUDIO_LIST):
                            print(f"\n===== 生成第 {idx+1} 个音频：{NOISY_AUDIO} =====")
                            ...
                            duration = (audio_length + hop_length - 1) // hop_length

                            # 5. Qwen 特征
                            with torch.no_grad():
                                qwen_feat = extract_qwen_feat(wav).unsqueeze(0).to(DEVICE)
                                target_len = duration
                            ...
                            # 6. 生成
                            print(f"样本{idx+1} | duration={duration}, text:{final_text_list}")
                            with torch.no_grad():
                                gen, _ = unwrapped_model.sample(
                                    cond=cond,
                            ...
                            # 7. 保存（自动编号 sample1, sample2...）
                            gen = gen.to(torch.float32).permute(0, 2, 1)
                            gen_audio = vocoder.decode(gen).cpu()

                            torchaudio.save(f"{log_samples_path}/update_{global_update}_sample{idx+1}_enhanced.wav", gen_audio, target_sr)
                            torchaudio.save(f"{log_samples_path}/update_{global_update}_sample{idx+1}_noisy.wav", wav.cpu(), target_sr)
