...
def extract_qwen_feat(audio_24k):
    ...
    # ==============================================
    # 🔥 强制补 0 到 1000 帧（你要的功能）
    # ==============================================
    input_features = feats["input_features"]  # shape: (1, 128, T)
    current_len = input_features.shape[-1]
    target_len = 1000  # 直接固定补到 1000

    # 时间维度后面补 0
    input_features = F.pad(
        input_features,
        (0, target_len - current_len),  # 只在最后一维（时间）补0
        value=0.0
    )

    # mask 长度也更新为 1000
    feature_lens = torch.tensor([target_len], dtype=torch.long, device=input_features.device)
    # ==============================================

    # 截取到 1000 帧
    input_features = input_features[0, :, :target_len]  # [128, 1000]
...
def collate_fn(batch):
    ...
    #     out = []
    #     for x in xs:
    #         pad = (0, 0, 0, max_len - x.shape[0])
    #         out.append(F.pad(x, pad, value=0))
    #     return torch.stack(out)

    def pad_qwen(xs, max_len):
        # 不补0！因为已经全部是固定长度 1000
