...
class IPSingleStreamBlockProcessor(nn.Module):
    ...
    def __call__(
        ...
        # 完全复用你原来的 IP 注意力逻辑
        qkv, mlp = torch.split(attn.linear1(norm), [3 * attn.hidden_size, attn.mlp_hidden_dim], dim=-1)
        q, k, v = rearrange(qkv, "B L (K H D) -> K B H L D", K=3, H=attn.num_heads, D=attn.head_dim)
        q, k = attn.norm(q, k, v)

        # 原始注意力
        attn_1 = attn.processor(q, k, v, mask, rope)
...
class DiTBlock(nn.Module):
    def __init__(
        ...
        if ip_context_dim is not None:
            self.ip_processor = IPSingleStreamBlockProcessor(
                context_dim=ip_context_dim,
                hidden_dim=dim
            )
    ...
    def forward(self, x, t, mask=None, rope=None,     
        ...
        # IP 融合注意力
        if self.ip_processor is not None and image_proj is not None:
            attn_output = self.ip_processor(self.attn, norm, mask, rope, image_proj, ip_scale)
        else:
            attn_output = self.attn(x=norm, mask=mask, rope=rope)
