...
def process_audio_file(audio_path, text, control_path, control_text, polyphone):
    
    if not Path(audio_path).exists():
        print(f"Main audio {audio_path} not found, skipping")
        return None
        
    if not Path(control_path).exists():
        print(f"Control audio {control_path} not found, skipping")
        return None
    ...
    try:
        ...
        return (audio_path, text, audio_duration, control_path, control_text)
...
def prepare_csv_wavs_dir(input_dir, num_workers=None):
    ...
    with graceful_exit():
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for i in range(0, len(audio_path_text_pairs), CHUNK_SIZE):
                ...
                chunk_futures = [executor.submit(process_audio_file, pair[0], pair[1], pair[2], pair[3], polyphone) for pair in chunk]
    ...
    raw_control_texts = [item[4] for item in processed]
    converted_control_texts = batch_convert_texts(raw_control_texts, polyphone, batch_size=BATCH_SIZE)
    ...
    for (audio_path, _, duration, control_path, _), conv_text, conv_ctrl_text in zip(processed, converted_texts, converted_control_texts):
        sub_result.append({"audio_path": audio_path, "text": conv_text, "duration": duration, "cond_audio_path": control_path, "cond_text": conv_ctrl_text})
        durations.append(duration)
        vocab_set.update(list(conv_text))
        vocab_set.update(list(conv_ctrl_text))
    ...
    with open(csv_file_path, mode="r", newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            if len(row) >= 4:
                ...
                text = row[1].strip()
                control_file = row[2].strip()
                control_text = row[3].strip()
                
                audio_file_path = parent / audio_file
                control_file_path = parent / control_file
                audio_text_pairs.append((audio_file_path.as_posix(), text, control_file_path.as_posix(), control_text))
