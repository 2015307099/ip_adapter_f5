...
#     #         # 只有老版 backbone checkpoint 才补 base_model
#     #         if (
...
#         #     if any(tok in new_key for tok in ("ref_encoder_block", "ip_adapters", "adapter")):
#         #         pass
#         #     else:
#         #         new_key = new_key.replace("transformer.", "transformer.base_model.", 1)
...
#         # #         # ✅ 核心修复：IP-Adapter 去掉 transformer. 前缀
...
#     control_audio_samples = control_audio.shape[-1]
...
#         control_audio_len = math.ceil(control_audio.shape[-1] / hop_length)
...
def infer_batch_process(
    ...
    def process_batch(gen_text):
        ...
        ref_audio_len = math.ceil(audio.shape[-1] / hop_length)
        control_audio_len = math.ceil(control_audio.shape[-1] / hop_length)    # 2s 24000Hz, 2s × 24000 = 48000, 48000 / 256 = 187.5, rounded up to 188. control_audio_len = 188
        if fix_duration is not None:
            # duration = int(fix_duration * target_sample_rate / hop_length)
            duration = control_audio_len
        else:
        ...
        with torch.inference_mode():
            generated, _ = model_obj.sample(
                cond=audio,
                control_cond=control_audio, #  (1, 48000)
                text=final_text_list,
                duration=duration,
                steps=nfe_step,
                cfg_strength=cfg_strength,
...
#         # Prepare the text
#         text_list = [gen_text]  # ====================== 修复：训练不用 ref_text + gen_text ======================
#         final_text_list = convert_char_to_pinyin(text_list)
...
#             # generated = generated[:, ref_audio_len:, :]  # 训练会去掉前缀，必须打开！
#             generated = generated.permute(0, 2, 1)
#             if mel_spec_type == "vocos":
#                 generated_wave = vocoder.decode(generated)
#             elif mel_spec_type == "bigvgan":
