...
from f5_tts.model.f5_ip_adapter_qwen import F5DiTWithIPAdapter
...
def main(model_cfg):
    ...
    qwen_encoder = Qwen3ASRAudioEncoder.from_qwen3_asr_pretrained(
        "/yangliusha03/panyuanhao/Qwen/Qwen3-ASR-1.7B",
        dtype=torch.float32,
        device=device
    )
    qwen_encoder.eval()
    for p in qwen_encoder.parameters():
        p.requires_grad = False

    model.qwen_encoder = qwen_encoder 
