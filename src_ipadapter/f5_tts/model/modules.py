...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, t, mask=None, rope=None, ip_k=None, ip_v=None):  # x: noised input, t: time embedding
        ...
        if ip_k is not None and ip_v is not None:
            # 正确提取 Q
            q = self.attn.to_q(norm)
            B, L, HD = q.shape
            H = self.attn.heads
            D = HD // H

            q = rearrange(q, "B L (H D) -> B H L D", H=H, D=D)
            ...
            # 计算 IP 交叉注意力
            ip_attn = F.scaled_dot_product_attention(q, ip_k, ip_v)
