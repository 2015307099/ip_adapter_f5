        ...
        self.transformer_blocks = nn.ModuleList(
            [
                DiTBlock(
                    ...
                    ip_context_dim=ip_context_dim,  # <-- 新增
                )
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            if self.checkpoint_activations:
                ...
                x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(block), x, t, mask, rope, image_proj, ip_scale,  # <-- 已传 IP 参数
                                                      use_reentrant=False)
            else:
                x = block(x, t, mask=mask, rope=rope, image_proj=image_proj, ip_scale=ip_scale)  # <-- 已传 IP 参数
